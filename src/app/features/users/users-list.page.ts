...
export default class UsersListPage implements OnInit {
  ...
  loadUsers() {
    ...
    this.userTenantRoleService.getUsersByTenant(tenantId).subscribe({
      next: (res) => {
        this.users.set(res || []);
  ...
  showInviteDialog() {
    ...
    if (ref) {
      ref.onClose.subscribe((result) => {
        if (result) {
          this.messageService.add({ severity: 'success', summary: 'Éxito', detail: 'Usuario invitado correctamente' });
          this.loadUsers();
        }
      });
    }
      ...
      accept: () => {
        ...
        if (tenantId) {
          this.userTenantRoleService.revokeAccess(tenantId, user.id).subscribe(() => {
            this.messageService.add({ severity: 'success', summary: this.transloco.translate('common.success'), detail: 'Usuario removido correctamente' });
            this.loadUsers();
          });
        }
      }
